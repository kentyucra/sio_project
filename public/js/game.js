...
function create() {
    ...
    this.socket.on('currentPlayers', players => {
        Object.keys(players).forEach(key => {
            if (players[key].playerId === this.socket.id) {
                addPlayer(this, players[key])
            } else {
                addOtherPlayers(this, players[key])
            }
        })
        console.log(players)
    })
...
function addPlayer(self, playerInfo) {
    self.ship = self.physics.add.image(playerInfo.x, playerInfo.y, 'ship').setOrigin(0.5).setDisplaySize(53, 40);
...
function update() {
    if (this.ship) {
        ...
        if (this.ship.oldPosition && (this.ship.x !== this.ship.oldPosition.x || this.ship.y !== this.ship.oldPosition.y || this.ship.r !== this.ship.oldPosition.rotation)) {
